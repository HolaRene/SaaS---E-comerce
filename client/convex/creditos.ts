...
export const getEstadisticasCreditos = query({
  ...
  handler: async (ctx, args) => {
    ...
    const totalAdeudado = creditosActivos.reduce((sum, c) => sum + c.saldoActual, 0);
    const promedioCliente = creditosActivos.length > 0 
      ? totalAdeudado / creditosActivos.length 
      : 0;
