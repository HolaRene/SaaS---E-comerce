...
export const getClientesByTienda = query({
  args: {
    ...
    segmento: v.optional(v.union(v.literal("frecuente"), v.literal("ocasional"), v.literal("mayorista"))),
  ...
  handler: async (ctx, args) => {
    ...
    if (args.estado && args.estado !== "todos") {
      if (args.estado === "fiado") {
        ...
        const clientesConCredito = new Set(creditosActivos.map((c) => c.clienteId));
  ...
  handler: async (ctx, args) => {
    ...
    const clientes = await Promise.all(
      clienteIds.map(async (id) => {
        ...
        return cliente ? { ...cliente, creditoId: credito?._id, saldoActual: credito?.saldoActual } : null;
      })
    );
...
export const updateSegmento = mutation({
  args: {
    clienteId: v.id("clientes"),
    segmento: v.union(v.literal("frecuente"), v.literal("ocasional"), v.literal("mayorista")),
  },
  handler: async (ctx, args) => {
    const identity = await ctx.auth.getUserIdentity();
    if (!identity) throw new Error("No autenticado");
