...
export const getRecordatoriosProximos = query({
  ...
  handler: async (ctx, args) => {
    ...
    const fechaLimite = new Date(
      hoy.getTime() + diasAdelante * 24 * 60 * 60 * 1000
    )
      ...
      if (user) {
        await ctx.scheduler.runAfter(
          ...
          internal.notificaciones.crearNotificacionUsuario,
          {
            usuarioId: user._id,
            tipo: 'recordatorio',
            titulo: args.tipo === 'cobro' ? 'Aviso de Cobro' : 'Recordatorio',
            mensaje:
              args.mensaje || `Tienes un pago pendiente de $${args.monto}`,
            tiendaId: args.tiendaId,
            // URL hardcoded como solicitó el usuario: mi-tienda/[id]/clientes (aunque para el cliente sería ver SU crédito)
            // Mejor dirigirlo a sus compras/creditos
            url: `/user/compras`,
          }
        )
      }
...
export const crearRecordatoriosAutomaticos = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const hoy = new Date()
    const fechaLimite = new Date(
      hoy.getTime() + diasAnticipacion * 24 * 60 * 60 * 1000
    )
